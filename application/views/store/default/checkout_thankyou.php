...
<body class="body-bgcolor">
		...
		<div class="row top-head">
			<div class="col-sm-2 text-center back-button">
				<a href="<?= base_url('store/order') ?>">
					<img src="<?= base_url('assets/store/default/img/back-button.png') ?>" class="img-fluid"
						alt="<?= __('store.back') ?>"></a>
			</div>
			...
			<div class="col-sm-2 text-center print-button">
				<a class="no-print print" href="javascript:void(0);"><img
						src="<?= base_url('assets/store/default/'); ?>img/printer.png" class="img-fluid"
						alt="<?= __('store.print') ?>"></a>
	...
	<div>
		<nav class="navbar navbar-expand-lg navbar-light bg-light mt-0 mr-1">
			...
			<button class="navbar-toggler" type="button" data-toggle="collapse" data-target="#profilesubnav"
				aria-controls="profilesubnav" aria-expanded="false" aria-label="Toggle navigation">
			...
			<div class="container main-container">
				...
				<div class="row">
					<h4>
						...
						<br>
						<span>
							<?= __('store.thank_you_for_purchasing_an_order') ?>
						</span>
					</h4>
				</div>
				<!--order number info-->
							...
							<tbody>
								<?php foreach ($products as $key => $product) { ?>
									<tr>
										<td>
											...
											<?php if ($order['status'] == 1 && ($product['product_type'] == 'downloadable' || $product['product_type'] == 'video' || $product['product_type'] == 'videolink') && $product['downloadable_files']) {
												if ($product['product_type'] == 'video' || $product['product_type'] == 'videolink') { ?>
													...
													<span>
														<a href="<?= base_url('store/vieworderdetails/') . $order['id'] . '?referance=' . $product['product_id'] ?>"
															title="<?= __('store.start_course') ?>" target="_blank">
												...
												<?php } else { ?>
													...
													<?php foreach ($product['downloadable_files'] as $downloadable_filess) {
														...
														?>
														<span>
															<a href="<?php echo $downloadable_link; ?>" target="_blank">
										...
										<td>
											<img class="img-thumbnail img-fluid" width="60"
												src="<?= (!empty($product['image'])) ? $product['image'] : base_url('assets/store/default/img/1.png'); ?>"
												alt="<?= __('store.image') ?>">
					...
					<div class="table-responsive-lg">
						<table class="table table-striped">
							<thead class="thead-dark">
								<tr>
									...
									<th scope="col">
										<?= __('store.transaction_id') ?>
									</th>
									<th scope="col">
										<?= __('store.payment_status') ?>
									</th>
								</tr>
							</thead>
							<tbody>
								<tr>
									...
									<?php foreach ($payment_history as $key => $value) { ?>
										<td>
											<?php echo str_replace("_", " ", $value['payment_mode']) ?>
										</td>
										<td>
											<?php echo $order['txn_id']; ?>
										</td>
										<td>
											<?php echo $value['paypal_status'] ?>
										</td>
									<?php } ?>
						...
						<?php if ($orderProof) { ?>
							<label class="control-label"><b>
									<?= __('store.payment_proof') ?>
								</b>
								<a href="<?= $orderProof->downloadLink ?>" target='_blank'>:
									<?= __('store.download') ?>
								</a>
							</label>
						<?php } ?>
				...
				<?php if ($order['files']) { ?>
					<div class="row non-flex">
						<h3 class="title">
							<?= __('store.order_attechments_download') ?>
						</h3>
						<div class="table-responsive-lg">
							<table class="table table-striped">
								<thead class="thead-dark">
	...
	<div class="container-fluid">
		<div class="row top-head footer">
			<div class="col-sm-12 text-center back-button">
				<p>
					<?= ($store_setting['footer'] != '') ? $store_setting['footer'] : __('store.all_rights_reserved') . " " . date('Y') . "." ?>
				</p>
			</div>
		</div>
	</div>
