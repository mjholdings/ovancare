...
class Product_model extends MY_Model
{
    ...
    {
        return $this->db->query('SELECT p.product_name, p.product_msrp, p.product_price, p.product_short_description, p.product_avg_rating, p.product_featured_image, p.product_slug FROM product p WHERE p.popular = 1 ORDER BY RAND() LIMIT 50')->result_array();
    }
...
//Wallet setting
    ...
    LEFT JOIN countries ON countries.id = users.Country
    WHERE wallet.status > 0 AND wallet.withdraw_request = 0 AND wallet.type NOT IN("vendor_sale_commission") AND users.type = "user" ' . $where . '
